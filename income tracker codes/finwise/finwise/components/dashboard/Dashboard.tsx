...
const Dashboard: React.FC<DashboardProps> = ({ session }) => {
    ...
    const [modal, setModal] = useState<'transaction' | 'category' | 'import' | 'export' | 'confirmDelete' | 'transactionDetail' | null>(null);
    ...
    const [animatedTx, setAnimatedTx] = useState<{id: string, type: 'new' | 'edit'} | null>(null);
    ...
    const filteredTransactions = useMemo(() => {
        return transactions
            ...
            .filter(t => {
                ...
                // Universal search through multiple fields
                const matchesDescription = t.description.toLowerCase().includes(search);
                const matchesCategory = (t.categories?.name || '').toLowerCase().includes(search);
                const matchesAmount = t.amount.toString().includes(search);
                const matchesDate = t.date.includes(search) || format(new Date(t.date), 'dd MMM yyyy').toLowerCase().includes(search);

                return matchesDescription || matchesCategory || matchesAmount || matchesDate;
            });
    }, [transactions, filterType, searchTerm]);

    const handleSave = (savedTransaction: Transaction, isNew: boolean) => {
        fetchData(); // Refetch all data to ensure consistency with totals, etc.
        setModal(null);
        setSelectedTransaction(null);
        if (savedTransaction) {
            setAnimatedTx({ id: savedTransaction.id, type: isNew ? 'new' : 'edit' });
            setTimeout(() => {
                setAnimatedTx(null);
            }, 2500); // Clear animation state after it has played
        }
    };
    ...
    const confirmDeleteItem = async () => {
        ...
        setModal(null); // Close confirm modal immediately

        if (itemToDelete.type === 'transaction') {
            const transactionId = itemToDelete.data.id;
            setDeletingTxId(transactionId); // Trigger fade-out animation

            setTimeout(async () => {
                const { error } = await supabase.from('transactions').delete().eq('id', transactionId);
                if (error) {
                    setError(error.message);
                    setDeletingTxId(null); // Reset on error
                } else {
    ...
    const getTransactionAnimationClass = (transactionId: string): string => {
        if (deletingTxId === transactionId) {
            return 'animate-fade-out-and-slide';
        }
        if (animatedTx?.id === transactionId) {
            return animatedTx.type === 'new' ? 'animate-slide-in-down' : 'animate-highlight';
        }
        return '';
    };
    
    const renderTransactionCard = (t: Transaction) => (
        <div key={t.id} className={`bg-surface dark:bg-dark-surface p-4 rounded-lg border border-border dark:border-dark-border flex flex-col gap-4 shadow-sm ${getTransactionAnimationClass(t.id)}`}>
            {/* Top row: Description and Amount */}
            <div 
                className="flex justify-between items-start gap-4 cursor-pointer"
                onClick={() => handleViewTransaction(t)}
                role="button"
                aria-label={`View details for ${t.description}`}
            >
    ...
    return (
        <div className="min-h-screen">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
                        <div className="relative w-full sm:max-w-xs">
                            ...
                            <input
                                ...
                                className="w-full p-2 pl-10 pr-10 border border-border dark:border-dark-border rounded-md bg-background dark:bg-dark-background focus:ring-2 focus:ring-primary"
                            />
                            ...
                            {searchTerm && (
                                <button
                                    ...
                                    className="absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-full text-text-secondary hover:text-text-primary dark:hover:text-dark-text-primary hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors"
                               ...
                               <table className="w-full text-left">
                                    ...
                                    <tbody>
                                        {filteredTransactions.length > 0 ? filteredTransactions.map(t => (
                                            <tr key={t.id} 
                                                className={`border-b border-border dark:border-dark-border last:border-b-0 hover:bg-gray-50 dark:hover:bg-slate-800/50 cursor-pointer ${getTransactionAnimationClass(t.id)}`}
                                                onClick={() => handleViewTransaction(t)}
                                            >
                                                ...
                                                <td className="p-3">
                                                    <div className="flex justify-center items-center gap-2">
                                                        <button 
                                                            onClick={(e) => { e.stopPropagation(); handleOpenTransactionModal(t); }} 
                                                            className="p-1 text-text-secondary dark:text-dark-text-secondary hover:text-primary rounded-full hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors"
                                                            aria-label={`Edit ${t.description}`}
                                                        ><EditIcon /></button>
                                                        <button 
                                                            onClick={(e) => { e.stopPropagation(); handleDeleteRequest(t, 'transaction'); }} 
                                                            className="p-1 text-text-secondary dark:text-dark-text-secondary hover:text-danger rounded-full hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors"
            ...
            <TransactionDetailModal
                isOpen={modal === 'transactionDetail'}
                onClose={() => setModal(null)}
                transaction={selectedTransaction}
